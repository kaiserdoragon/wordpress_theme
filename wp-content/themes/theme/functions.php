...
function allsite_style_script()
{
  ...
  //リセットcss
  wp_register_style('reset', get_template_directory_uri() . '/css/reset.min.css', array());
  wp_enqueue_style('reset');

  //プラグインやwebフォントなど追加のCSSはこの辺に書きます。

  //カスタムcss
  wp_register_style('custom', get_template_directory_uri() . '/css/style.min.css', array());
...
function header_style_script()
{
  if ($GLOBALS['pagenow'] != 'wp-login.php' && !is_admin()) {
    //テーマ用のjsファイルを読み込み
    wp_register_script('mainscripts', get_template_directory_uri() . '/js/scripts.min.js', array('jquery'));
